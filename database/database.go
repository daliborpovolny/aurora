...
func Seed(q *database.Queries) {
	ctx := context.Background()
	if users, _ := q.ListUsers(ctx); len(users) == 0 {
		...
		_, err := q.CreateParent(ctx, 1)
		if err != nil {
			panic(err)
		}

		teacher, err := q.CreateTeacher(ctx, 2)
		if err != nil {
			panic(err)
		}
		...
		q.CreateStudent(ctx, database.CreateStudentParams{
			UserID:  3,
			ClassID: class.ID,
		})
		...
		q.AssignStudentToParent(ctx, database.AssignStudentToParentParams{
			ParentID:  1,
			StudentID: 1,
		})
	}
